...
class DBQuery:
    ...
    def get_team_stats_better_than_average(self, input_team_name):
        ...
        query = f"""SELECT s1.team, s1.stat_name, s1.stat_value FROM lironcohen3.stats AS s1
                    WHERE
...
if __name__ == '__main__':
    ...
    dbq = DBQuery()
    dbq.get_teams_that_won_against_odds()
    dbq.get_top_position_players_stats("QB")
    dbq.get_team_stats_better_than_average("Miami")
    dbq.get_max_capacity_venue_per_state()
    dbq.get_min_max_weights_heights_per_draft_position()
    dbq.get_most_picked_college_teams_in_draft()
    dbq.close_conn()
